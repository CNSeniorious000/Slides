...
from pymouse import PyMouse; mouse = PyMouse()
import numpy as np
import numba as nb
import webp
...
class Scene(pg.sprite.RenderUpdates):
    cache = {}

    def __new__(cls, *args, **kwargs):
        try:
            return Scene.cache[args, kwargs]
        except KeyError:
            ans = Scene.cache[args, kwargs] = super().__new__(cls, *args, **kwargs)
            return ans
...
class UI:
    def __init__(self, screenx, screeny, flags=pg.NOFRAME, bgd=None):
        ...
        self.drag = False
        self.x = (display_x-self.screenx)//2
        self.y = (display_y-self.screeny)//2

        if bgd:
            if isinstance(bgd, pg.Surface):
                self.screen.blit(bgd, (0,0))
            elif isinstance(bgd, np.ndarray):
                pg.surfarray.blit_array(self.screen, bgd)
            elif isinstance(bgd, (tuple, list)):
                self.screen.fill(bgd)
            elif isinstance(bgd, int):
                pg.surfarray.pixels3d(self.screen)[:] = bgd
            else:
                raise NotImplementedError
        else:
            self.screen.fill((243, 243, 243))
        self.bgd = self.screen.convert()
    ...
    def move_window(self, x=None, y=None):
        x = (display_x-self.screenx)//2 if x is None else x
        y = (display_y-self.screeny)//2 if y is None else y

        ctypes.windll.user32.MoveWindow(self.hwnd, x, y, self.screenx, self.screeny, False)
